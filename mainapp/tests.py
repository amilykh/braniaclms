...
class TestNewsPage(TestCase):
    ...
    def setUp(self):
        ...
        self.user_admin = authapp_models.User.objects.get(
            username="admin")
...
class TestCoursesWithMock(TestCase):
    ...
    def test_page_open_detail(self):
        course_obj = mainapp_models.Courses.object.get(pk=1)
        path = reverse("mainapp:courses_detail", args=[course_obj.pk])
        with open("mainapp/fixtures/005_feedback_list_1.bin", "rb"
                  ) as inpf, mock.patch("django.core.cache.cache.get"
                                        ) as mocked_cache:
